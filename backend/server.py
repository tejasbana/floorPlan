import os
from boundary import fit_boundary
import numpy as np
...
def upload():
    if request.method == 'POST' :
        ...
        if True:#'photo' in request.files:
            ...
            # save from url 
            url = "https://firebasestorage.googleapis.com/v0/b/floorplan-firebase.appspot.com/o/images%2Fbg.jpg?alt=media&token=512c5d6a-3088-4c2e-ba44-cfebd76c8773"
            response = requests.get(url)
            img = Image.open(BytesIO(response.content)).convert("RGB")
            boundary_name = "./generated/boundary" + ".png"
            img.save(boundary_name, "jpeg")

            final_list = []
            for i in range(num_variations):
                item = fit_boundary("./generated/plan{}.png".format(i), boundary_name)
                if item:
                    final_list.append([item,i])
            final_list.sort()
            final_list.reverse()
            print(final_list)  
            for idx,pair in enumerate(final_list):
                i = pair[1]
                bucket = storage.bucket()
                blob = bucket.blob('floorplan/plan{}.jpg'.format(idx))
